...
const Register:React.FC<RegisterProps> = ({
  ...
  const [optVerification, setOptVerification] = useState<boolean>(false);
  const [otpVerified, setOtpVerified] = useState<boolean>(false);
  const [genratedOtp, setGenratedOtp] = useState<string>("");
  const [otpMessage, setOptMessage] = useState<string>("");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setGenratedOtp(genOtp);
    //sendOtp
    const result = await sendOtp(email, genOtp);
    console.log(result, "result of otp sent");
    if (result.success) {
        ...
        <div>
          ...
          <CardContent className="w-full flex justify-center">
            <Otp
              ...
              setOptVerification={setOptVerification}
              email={email}
              password={password}
              setMessage={setMessage}
              setOptMessage={setOptMessage}
              setIsLogin={setIsLogin}
            />
          </CardContent>
          <CardFooter className="w-full flex justify-center text-destructive">
            <p>{otpMessage && otpMessage}</p>
          </CardFooter>
        </div>
      ) : (
        <div>
            ...
            >
              ...
              <p className="w-full text-center">
                ...
                <span
                  ...
                  onClick={() => setIsLogin ? setIsLogin(true)  : router.replace("/login")}
                >
                  Login
                </span>
              </p>
              {message && (
                <p className="w-full text-center text-destructive -mt-3">
                  {message}
                </p>
              )}
